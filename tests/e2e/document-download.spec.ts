...
test.describe('Document Download Flow', () => {
  ...
  test('should display download section correctly', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Télécharger les documents' })).toBeVisible()
    ...
    const letterDownloadBtn = page.getByRole('button', { name: /Télécharger Lettre/ })

    await expect(cvDownloadBtn).not.toBeVisible()
    await expect(letterDownloadBtn).not.toBeVisible()
  })

  test('should show format selection when documents are available', async ({ page }) => {
  ...
  test('should handle download workflow with mock data', async ({ page }) => {
    ...
    const downloadSection = page.locator('.document-download, [class*="download"]')
  ...
  test('should handle bulk download section when both documents exist', async ({ page }) => {
  ...
  test('should simulate complete download workflow', async ({ page }) => {
    ...
    await page.getByLabel('Job Description').fill('This is a test job description that is long enough to pass validation requirements for testing purposes.')

    // Check current state
    const analyzeButton = page.getByRole('button', { name: 'Analyze Job Offer' })
    const isDisabled = await analyzeButton.getAttribute('disabled')
    console.log('Job form filled, analyze button state:', isDisabled === null ? 'enabled' : 'disabled')
