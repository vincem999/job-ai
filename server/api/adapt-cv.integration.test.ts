...
describe('CV Adaptation Integration Tests', () => {
  ...
  describe('Data Sanitization', () => {
    it('should sanitize potentially malicious CV data', () => {
      ...
      const sanitizedCV = {
        ...
        workExperience: maliciousCV.workExperience.map(exp => ({
          ...exp,
      ...
      expect(sanitizedCV.workExperience[0].company).not.toContain('<script>')
      expect(sanitizedCV.workExperience[0].description).not.toContain('<script>')
      ...
      expect(sanitizedCV.workExperience[0].company).toContain('Evil Corp')
    })
    ...
    it('should preserve safe HTML entities', () => {
      ...
      const sanitizedSummary = sanitizeForDisplay(cvWithEntities.personalInfo.summary)
  ...
  describe('Date Transformation', () => {
    it('should handle date string to Date object conversion', () => {
      ...
      const transformedCV = {
        ...
        workExperience: cvWithDateStrings.workExperience.map(exp => ({
          ...exp,
      ...
      expect(transformedCV.workExperience[0].startDate).toBeInstanceOf(Date)
      expect(transformedCV.workExperience[0].endDate).toBeInstanceOf(Date)
  ...
  describe('Focus Areas Processing', () => {
    it('should handle valid focus areas', () => {
      const focusAreas = ['technical skills', 'leadership experience', 'achievements']

      const sanitizedFocusAreas = focusAreas.map(area => sanitizeForDisplay(area))
    ...
    it('should sanitize malicious focus areas', () => {
      ...
      const sanitizedFocusAreas = maliciousFocusAreas.map(area => sanitizeForDisplay(area))
  ...
  describe('Error Message Quality', () => {
    it('should provide clear error messages for validation failures', () => {
      ...
      if (!result.success) {
        ...
        expect(errorPaths.some(path => path.includes('cvData.personalInfo'))).toBe(true)
        expect(errorPaths.some(path => path.includes('jobAnalysis.requiredSkills'))).toBe(true)
      }
    })
  })
})