...
describe('CoverLetterRequestSchema validation', () => {
  it('should validate a complete cover letter request', () => {
    ...
    if (result.success) {
      ...
      expect(result.data.personalMessage).toBe('I am very interested in this position.')
  ...
  it('should reject request with missing required fields', () => {
    ...
    if (!result.success) {
      expect(result.error.issues.some(issue => issue.path.includes('jobAnalysis'))).toBe(true)
    }
  })

  it('should validate all available tone options', () => {
    const toneOptions = ['Professional', 'Friendly', 'Enthusiastic', 'Formal'] as const
...
describe('Cover letter data sanitization', () => {
  it('should sanitize personal message input', () => {
    const maliciousMessage = '<script>alert("xss")</script>I am interested in this role'
  ...
  it('should sanitize CV personal information', () => {
    ...
    const sanitizedFirstName = sanitizeForDisplay(maliciousCV.personalInfo.firstName)
    const sanitizedLastName = sanitizeForDisplay(maliciousCV.personalInfo.lastName)
  ...
  it('should handle complete workflow from request to validation', () => {
    ...
    const transformedBody = {
      ...
      cvData: {
        ...
        workExperience: requestBody.cvData.workExperience.map((exp: any) => ({
          ...exp,
    ...
    if (validationResult.success) {
      ...
      expect(data.cvData.workExperience[0].startDate).toBeInstanceOf(Date)
    }
  })
