...
const mockJobAnalysisResponse: JobAnalysisResponse = {
  requiredSkills: ['JavaScript', 'TypeScript', 'React', 'Node.js', 'PostgreSQL', 'MongoDB'],
...
const mockCVData: CVData = {
  ...
  personalInfo: {
    ...
    summary: 'Experienced software engineer with 4 years of full-stack development experience'
  },
  workExperience: [
    {
      id: 'exp-1',
...
const mockCoverLetterRequest: CoverLetterRequest = {
  ...
  personalMessage: 'I am particularly excited about the opportunity to work in fintech',
  tone: 'Professional'
}
...
describe('generateCVAdaptationPrompt', () => {
  it('should generate a valid CV adaptation prompt', () => {
    const prompt = generateCVAdaptationPrompt(mockCVData, mockJobAnalysisResponse)
  ...
  it('should include CV data and job analysis in structured format', () => {
    ...
    expect(prompt).toContain(mockCVData.workExperience[0].company)
...
describe('generateCoverLetterPrompt', () => {
  ...
  it('should handle different tones correctly', () => {
    const tones: Array<CoverLetterRequest['tone']> = ['Professional', 'Friendly', 'Enthusiastic', 'Formal']
...
describe('validatePromptTemplate', () => {
  ...
  it('should detect missing required elements in job analysis prompts', () => {
    ...
    expect(validation.issues.some(issue => issue.toLowerCase().includes('json') || issue.toLowerCase().includes('short'))).toBe(true)
  })

  it('should validate CV adaptation prompts correctly', () => {
    const validPrompt = generateCVAdaptationPrompt(mockCVData, mockJobAnalysisResponse)
  ...
  it('should detect prompts that are too short', () => {
    ...
    expect(validation.isValid).toBe(false)
    expect(validation.issues).toContain('Prompt is too short - may not provide sufficient context')
  })
  ...
  it('should detect prompts that are too long', () => {
    ...
    expect(validation.isValid).toBe(false)
    expect(validation.issues).toContain('Prompt is too long - may exceed token limits')
  })

  it('should validate different prompt types with specific requirements', () => {
    // Test CV adaptation specific validation
    const cvPromptWithoutOptimization = generateJobAnalysisPrompt(mockJobAnalysisRequest)
    const cvValidation = validatePromptTemplate('cvAdaptation', cvPromptWithoutOptimization)
    expect(cvValidation.isValid).toBe(false)
    expect(cvValidation.issues).toContain('CV adaptation prompt missing optimization elements')

    // Test cover letter specific validation
    const coverLetterPromptWithoutFormatting = generateJobAnalysisPrompt(mockJobAnalysisRequest)
    const coverLetterValidation = validatePromptTemplate('coverLetter', coverLetterPromptWithoutFormatting)
    expect(coverLetterValidation.isValid).toBe(false)
    expect(coverLetterValidation.issues).toContain('Cover letter prompt missing formatting requirements')
  })
})
...
describe('Prompt Integration Tests', () => {
  it('should generate coherent prompts for the complete workflow', () => {
    ...
    const cvPrompt = generateCVAdaptationPrompt(mockCVData, mockJobAnalysisResponse)
  ...
  it('should handle edge cases gracefully', () => {
    ...
    expect(() => generateCVAdaptationPrompt(minimalCVData, mockJobAnalysisResponse)).not.toThrow()
    expect(() => generateCoverLetterPrompt(minimalCoverLetterRequest)).not.toThrow()
  })
